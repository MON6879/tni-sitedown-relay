...
- **Đã cập nhật:**
  - `api/search_bot.py`: Bỏ prefix số thứ tự `i.` và tiêu đề phụ, gửi trực tiếp văn bản template Daily result và Daily plan.
  - `api/collector.py`: Bỏ prefix số thứ tự `i.` và toàn bộ dòng tiêu đề chú thích (`Template: ...`), gửi trực tiếp văn bản template (`/inventory`, `/order`, `/revoke`, `/move`, `/export`, `/done`).
