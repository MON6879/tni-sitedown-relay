...
- **Đã cập nhật:**
  ...
  - `api/refuel_collector.py`: Cập nhật câu hỏi tiếng Anh chuẩn template: `📢 @TeamLeader Who is assigned to follow monitor?`. Bỏ tag người gửi (`@sunil`), chỉ tag duy nhất Team Leader (`@PaingAung` cho Team 1, `@AungNaing` cho Team 2). Khắc phục lỗi `norm_chat_id` bị dư tiền tố `-100` giúp thu thập 100% báo cáo từ group `9 TNI REQUEST REFUEL`.
