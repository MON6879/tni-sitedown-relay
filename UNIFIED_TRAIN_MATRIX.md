...
| **Toa 1+11** | 📊 Reports 1-4a/4b + BOD | 05:48 & 15:48 MMT | Báo cáo Backlog, Cron & BOD. **Tách Report 4 thành 4a (Summary) và 4b (Full Report)** | `backlog_send.py` + `cron_send.py` + `daily_bod_assign.py` | Bot API Tokens $\rightarrow$ Telegram Groups | Tự động gửi báo cáo công việc hàng ngày |
| **Toa 1+11** | 📦 Standalone Asset 3.1 | 05:48 & 15:48 MMT | **Ghế R3.1**: Báo cáo tiến độ vật tư Asset độc lập (`3.1 Asset progress for material`) sử dụng **ô vuông nhiều màu cố định** (`🟦🟨🟩🟧🟥🟫🟪⬜`) cho từng loại action | `cron_send.py` (`build_asset_msg`) | Bot API Tokens $\rightarrow$ Telegram Groups | Tin nhắn riêng độc lập tổng hợp tiến độ xuất/nhập/vận chuyển vật tư Asset cho Control Site & 4 Teams |
...
| **Toa 12** | 🕵️ Ingestion Auditor (Ghế 9.1) | **Quét/Đếm: 01:03 AM MMT<br>Gửi Báo Cáo: 04:33 AM MMT** | Giám sát thu thập dữ liệu & ảnh Telegram $\rightarrow$ Soi Google Sheet. Quét lúc 01:03 AM & Gửi báo cáo lúc 04:33 AM về Group 9.1 | `ingestion_auditor.py` | Search Bot / Telethon $\rightarrow$ Group `9.1 Cross check Fuel 30'` (`-5469544739`) | Phát cảnh báo Tên Sheet / Tab không thu thập được thông tin |
| **Toa 12** | ⏱️ Live Sync Auditor (Ghế 9.2) | **Quét liên tục 30s (Realtime 24/7)** | **Ghế Chuyên Coi Cập Nhật**: Giám sát mốc thời gian cập nhật biểu mẫu & thống kê BI Portal. Tự động hiển thị nhãn `⚡ Realtime Synced from Google Sheets — [Timestamp] MMT` trên mọi bảng, biểu đồ, thẻ tóm tắt & tab BI. Tự động trích xuất động **Target Need Complete % (Ô F1, ví dụ 75%)** & **Wait CD Total (Cột F)** từ Sheet `Sum all WO Team` thay thế giá trị tĩnh cũ. | `api/bi_data.py` / `index.html` / `executive_dashboard.html` | Client JS 30s Direct CSV Feed $\rightarrow$ Group `9.1 Cross check Fuel 30'` (`-5469544739`) | Đảm bảo 100% biểu mẫu & thống kê luôn có nhãn thời gian cập nhật thực tế và tự động làm mới khi Google Sheet thay đổi |
