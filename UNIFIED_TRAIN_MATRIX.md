...
| **Toa 2** | 📦 Asset & Material Order Collector | **Dãy A**: Đơn đăng ký vật tư & tài sản | Col A: User ID<br>Col B: Form Data<br>Col C: Timestamp | Telegram Bot `@TNIASSETorderREQUEST_BOT` $\rightarrow$ Vercel | `api/collector.py` $\rightarrow$ `REFUEL_APPS_SCRIPT_URL` | **Dòng 2 (`insertRowsBefore(2)`)** |
