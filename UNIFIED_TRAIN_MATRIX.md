...
| **Toa Auditor** | 👑 System Master Sentinel (Ghế AUDITOR-9.1) | **09:00 AM MMT hàng ngày & Kích hoạt khi cần** | Quét kiểm toán toàn diện: Schedule Adherence (Đúng giờ), Deduplication (Chống nhân đôi), Webhooks, GAS, Sheets. Nếu OK: Gửi `🟢 [AUDITOR-9.1] 1, 2, 3, 4 OK`. Nếu có lỗi: Chỉ hiển thị chi tiết sự cố | `system_auditor.py` | Telethon + Bot API $\rightarrow$ Gửi duy nhất về Telegram DM Admin (`6859790680`) | Quét kiểm toán sâu toàn bộ hệ thống |
| **ĐỘC LẬP** | 🟢 1. CRON ĐỘC LẬP (Site Down Relay) | Phút `:06` & `:36` hàng giờ (Khởi động :03 & :33 UTC, sleep chờ đúng `:06:00` & `:36:00` MMT) | Cào tin trạm sập NOC Pro CHÍNH THỨC | `botlookup_relay.yml` | Telethon `@Phongha79` $\rightarrow$ Sheet Site Down Row 2 | Cào tin trạm sập NOC Pro CHÍNH THỨC đúng chính xác phút :06 và :36 MMT |
