...
function processSummaryAwAz(sheet) {
  ...
  // 🛑 2. NẾU TIN CŨ (Ví dụ mốc giờ quá 30 phút so với giờ hiện tại) THÌ BỎ QUA KHÔNG GỬI
  if (!isTimestampFresh_(tsKey, 0.5)) {
    Logger.log("[Luồng AW7] ⚠️ Timestamp AW7 quá cũ (" + tsKey + ") so với giờ hiện tại → Bỏ qua không gửi tin cũ.");
    return false;
  }
...
function isTimestampFresh_(tsStr, maxAgeHours) {
  maxAgeHours = maxAgeHours || 3;
  ...
  if (nowParts) {
    ...
    const diffHours = (curDate.getTime() - tsDate.getTime()) / (1000 * 60 * 60);

    if (diffHours > maxAgeHours || diffHours < -0.5) {
      return false; // Tin cũ (ví dụ 07:22 sáng khi hiện tại là 17:35 chiều) hoặc ngày khác
    }
  }
  return true;
}
