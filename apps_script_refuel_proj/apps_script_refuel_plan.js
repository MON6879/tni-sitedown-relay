...
function extractTeamName(text) {
  if (!text) return "";
  // Match team02, Team03, Team-2, Team 2, TEAM 1, team_4, Team X, Aung Naing Refuel Team 2, etc.
  const m = text.match(/(?:team|team\s*id|đội)[\s\-_]*(\d+|[a-z0-9]+)/i);
  if (m) {
    const val = m[1].trim();
    if (/^\d+$/.test(val)) {
      return "Team " + parseInt(val, 10);
    }
    return "Team " + val.toUpperCase();
  }
  return "";
}

function extractDateVal(text, defaultDate) {
  if (!text) return defaultDate;
  // Match dd/mm/yyyy, d/m/yyyy, dd.mm.yyyy, d.m.yyyy, dd-mm-yyyy, d-m-yyyy
...
function collectMessage(body) {
  ...
  if ((textLower.includes("name of ft staff member") && textLower.includes("supervise")) || 
    ...
    const dateVal = extractDateVal(text, today);
    const ftName  = extractFtName(text);
  ...
  if (textLower.includes("plan")) {
    ...
    const dateVal = extractDateVal(text, today);
    const teamVal = extractTeamName(text);
