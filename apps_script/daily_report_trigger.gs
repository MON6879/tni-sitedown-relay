...
function triggerDailyReport_() {
  ...
  if (!token) {
    Logger.log('❌ GITHUB_TOKEN chua duoc set trong Script Properties!');
  ...
  try {
    ...
    if (code === 204) {
      Logger.log(`✅ [${now}] Reports 1-4 triggered thanh cong!`);
    } else {
      Logger.log(`⚠️ [${now}] GitHub API: HTTP ${code}`);
      Logger.log(resp.getContentText().substring(0, 300));
    }
  } catch (e) {
    Logger.log('❌ Loi UrlFetchApp: ' + e.message);
  }
}
