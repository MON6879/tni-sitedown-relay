...
def get_control_note_from_sheet() -> str:
    """Đọc ô H1:H3 từ tab Config của Master Sheet 1Etd2PmbY5LgPaYhkdykT7KYXZHhB-_Qx3u-UXhFgpI8."""
    try:
        resp = requests.get(CONFIG_SHEET_URL, headers={"User-Agent": "Mozilla/5.0"}, timeout=20)
        resp.raise_for_status()
        df = pd.read_csv(io.StringIO(resp.text), header=None, dtype=str, on_bad_lines="skip")
    ...
    if SEND_BOT_TOKEN:
        ...
        for team_key in ordered_teams:
            ...
            for idx, chk in enumerate(chunks, 1):
                p_lines = []
                # MỖI PHẦN ĐỀU CÓ TIÊU ĐỀ BÁO CÁO ĐỂ TELETHON SCAN KHỚP VÀ XÓA SẠCH
                if total_parts > 1:
                    part_title = f"📋 4. Report — Daily EOD Task & Stats — {t_name} (Part {idx}/{total_parts})"
                else:
